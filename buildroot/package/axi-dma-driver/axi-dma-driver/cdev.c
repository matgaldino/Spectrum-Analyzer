...
static long axi_dma_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  switch(cmd)
  {
    ...
    case AXI_DMA_GET_REGISTER_VALUE:
      rc = copy_from_user(&reg_offset, (void*)arg, sizeof(uint32_t)); // on récupère l'offset du registre à lire depuis l'arguement user
      if(rc < 0)
      {
        dev_err( &chan->parent->pdev->dev
      ...
      rc = copy_to_user(&arg, (void*)&reg_value, sizeof(uint32_t)); // on écrit la valeur du registre dans l'argument user
      if(rc < 0)
      {
        dev_err( &chan->parent->pdev->dev
